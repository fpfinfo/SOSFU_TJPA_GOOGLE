...
import { PRESTACAO_STATUS_COLORS } from "@/components/status/PrestacaoStatusMachine";

export default function ReembolsoStatusTimeline({ reembolsoId }) {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const data = await HistoricoStatusReembolso.filter({ reembolso_id: reembolsoId }, "-created_date");
      setHistorico(data);
    } finally {
      setLoading(false);
    }
