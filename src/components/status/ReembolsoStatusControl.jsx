...
import { getReembolsoAvailableTransitions, changeReembolsoStatus, REEMBOLSO_STATUS_DESCRIPTIONS } from "./ReembolsoStatusMachine";
import { useCurrentUser } from "@/components/utils/UserContext";

export default function ReembolsoStatusControl({ reembolso, onStatusChanged, disabled = false }) {
  ...
  if (!user || !reembolso) return null;

  const availableTransitions = getReembolsoAvailableTransitions(reembolso.status, user.role);
  ...
  const handleConfirmChange = async () => {
    ...
    setError("");
    try {
      await changeReembolsoStatus(reembolso.id, selectedStatus, "", user.id || user.email, user.role, user.email, user);
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              Alterar Status do Reembolso
            </DialogTitle>
          </DialogHeader>

          <div className="space-y-4 py-4">
            <div className="text-sm text-gray-600">
              <p><strong>Status atual:</strong> {reembolso.status.replace(/_/g, " ")}</p>
              <p><strong>Novo status:</strong> {selectedStatus.replace(/_/g, " ")}</p>
            </div>

            <div className="p-3 bg-blue-50 rounded-lg text-sm text-blue-800">
              {REEMBOLSO_STATUS_DESCRIPTIONS[selectedStatus]}
            </div>
